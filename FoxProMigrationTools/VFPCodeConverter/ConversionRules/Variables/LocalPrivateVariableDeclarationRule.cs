...
{
    public class LocalPrivateVariableDeclarationRule : ConversionRuleBase, IConversionRule
    {
        #region Constructor

        public LocalPrivateVariableDeclarationRule()
        {
            RuleName = "LocalPrivateVariableDeclarationRule";
            RuleType = "LocalPrivateVariableDeclaration";
        }
        #endregion
        ...
        public string ApplyConversionRule(string sourceCode, ConversionParameters conversionParameters)
        {
            ...
            if (match.Success)
            {
                ...
                string variableNames = match.Groups["variableNames"].ToString();
                ...
                foreach (var variableNameWithSpace in variableNames.Split(','))
                {
                    string variableName = variableNameWithSpace.Trim();
                    Utility.ConvertVariables(conversionParameters, groupBuilder, convertedCodeBuilder, variableName, leadingSpace, false);
                }
